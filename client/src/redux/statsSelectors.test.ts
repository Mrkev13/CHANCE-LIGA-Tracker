...
describe('statsSelectors', () => {
  ...
  it('selects top scorers correctly', () => {
    ...
    expect(scorers[0]).toMatchObject({ id: 'p1', name: 'Player 1', count: 2 });
  });

  it('selects top assists correctly', () => {
    const assists = selectTopAssists(mockState);
    expect(assists).toHaveLength(1);
    expect(assists[0]).toMatchObject({ id: 'p2', name: 'Player 2', count: 1 });
  });

  it('selects yellow cards correctly', () => {
    const cards = selectTopYellowCards(mockState);
    expect(cards).toHaveLength(1);
    expect(cards[0]).toMatchObject({ id: 'p3', name: 'Player 3', count: 1 });
  });

  it('aggregates stats for players with same name but different IDs', () => {
        ...
        events: [
          {
            ...
            minute: 20,
            team: 'home',
            player: { id: 'p_random_123', name: 'Chorý' }
          }
        ]
      }
    ];
